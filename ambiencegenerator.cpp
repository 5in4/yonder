    ...
    ui(new Ui::AmbienceGenerator)
{
    ...
    connect(editor_frame, SIGNAL(deactivated()), generator_frame, SLOT(refreshSoundUi()));

    // Pause everything when editor is launched
    connect(editor_frame, SIGNAL(activated()), this, SLOT(ambienceEditor()));
    // Reload project when editor is deactivated
...
void AmbienceGenerator::setProject(QString project_path) {
    // For security, check if project path really really exists
    ...
    connect(editor_frame, SIGNAL(deactivated()), generator_frame, SLOT(hotkeysCreateButtons()));
...
void AmbienceGenerator::webappStart() {
    ...
    webapp = new Webapp(settings.value("Settings/webapp_port").toInt(), 128, QString(":webservice/static"), this);
...
void AmbienceGenerator::resizeEvent(QResizeEvent *event) {
